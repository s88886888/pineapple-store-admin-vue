    ...
    <div>
      <el-card class="box-card-GoodsSkuTable" v-show="GoodsSKuShow">
        ...
        <el-table :data="goodsSkuTable" style="width: 100%">
          ...
          <el-table-column prop="skuId" label="库存ID" width="120" />
            ...
            <template #default="scope">
              <div>
                <el-button
                  type="danger"
                  size="small"
                  @click="delSku(scope.row.skuName, scope.row.skuId)"
                  >删除</el-button
                >
                <el-button
                  v-if="editSkuShow"
...
async function getSkuList() {
  await addGoods
    .getGoodsSkuByItemId({ Id: editProductId.value })
    .then((res) => {
      if (res.code == 200) {
        for (let i in res.data) {
          goodsSkuTable.push({
            skuId: res.data[i].skuId,
            skuName: res.data[i].skuName,
            originalPrice: res.data[i].originalPrice,
            sellPrice: res.data[i].sellPrice,
            discounts: res.data[i].discounts,
            stock: res.data[i].stock,
            status: res.data[i].status,
            skuStar: res.data[i].skuStar,
          });
        }
      }
    });
...
const addGoodsRules = reactive<FormRules>({
  productName: [
    ...
    { min: 2, max: 10, message: "大小控制在  2 to 10 字节", trigger: "blur" },
  ],
  content: [
      ...
      if (activeSteps.value == 2) {
        if (goodsSkuTable.length == 0) {
          ...
          const skuStarIndex = goodsSkuTable.findIndex(
            (item: { skuStar: number }) => item.skuStar == 1
          );
...
let goodsSkuTable = reactive<SkuType[]>([]);
